...
std::vector<Inactive_period*> inactive_periods_list;
...
std::vector<Inactive_period*> offloeded_local_intervals;
...
//TODO: You should write a mini compiler pass to fill the liveness information of every tensor (liveness information)
...
//TODO: You should write another compiler pass to fill all the inactive periods for every tensor (inactive_periods)
                ...
                for (int j = current_tensor->live_interval[0]; j < current_tensor->live_interval[1]; j++)
                {
                    ...
                    if (kernel_list[j].inputs.find(current_tensor)!=kernel_list[j].inputs.end() || kernel_list[j].outputs.find(current_tensor)!=kernel_list[j].outputs.end())
                    {
                        if (!a_interval_started)
                        {
                            ...
                            {
                                //Start one interval
                                Inactive_period* new_interval = new Inactive_period(current_tensor);
                        ...
                        {
                            inactive_periods_list.back()->kernelLevel_interval[1] = j;
            ...
            bool a_interval_started = false;
                ...
                for (int j = 0; j <= one_use; j++)
                {
                    ...
                    if (kernel_list[j].inputs.find(current_tensor)!=kernel_list[j].inputs.end() || kernel_list[j].outputs.find(current_tensor)!=kernel_list[j].outputs.end())
                    {
                        ...
                        {
                            inactive_periods_list.back()->kernelLevel_interval[1] = j;
                            if (j < inactive_periods_list.back()->kernelLevel_interval[0])
                            {
...
void Tensor::print_inactive_periods(){
    ...
    {
        std::cout<<"interval "<<i<<": "<<inactive_periods[i]->kernelLevel_interval[0]<<"--------"<<inactive_periods[i]->kernelLevel_interval[1]<<std::endl;
...
//A provided compiler pass to calculate the estimated execution time for every tensors' inactive period length(time)
void get_inactive_periods_time(){
    ...
    for (int i = 0; i < inactive_periods_list.size(); i++)
    {
        ...
        {
            assert(inactive_periods_list[i]->kernelLevel_interval[1] > inactive_periods_list[i]->kernelLevel_interval[0]);
            inactive_periods_list[i]->time_estimated = kernel_time_table[inactive_periods_list[i]->kernelLevel_interval[1]] - kernel_time_table[inactive_periods_list[i]->kernelLevel_interval[0]];
        }
        else
        {
            assert(inactive_periods_list[i]->kernelLevel_interval[1] < inactive_periods_list[i]->kernelLevel_interval[0]);
            int end = inactive_periods_list[i]->kernelLevel_interval[1];
            int start = inactive_periods_list[i]->kernelLevel_interval[0];
            end += kernel_num;
            inactive_periods_list[i]->time_estimated = kernel_time_table_extended[end] - kernel_time_table_extended[start];
        }
    }
